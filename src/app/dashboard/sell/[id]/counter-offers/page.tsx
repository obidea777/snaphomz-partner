...
const Offers = () => {
  ...
  const { data: propertyOffers, isLoading: offerLoading, error ,refetch } = useGetPropertyOffersByProperty(claimedProperty?.id?.toString(), claimedProperty?.listingid?.toString());
  const handleOfferClick = (offer: IBuyer) => {
    setSelectedOffer(offer)
    router.push(`/offers/${offer.id}?type=offer`)
  }
  const { data: propertyCounterOffers, isLoading: counterOfferLoading , refetch:refetchCoutnerOffer } = useFetchPropertyCounterOffers(claimedProperty?.id?.toString(), claimedProperty?.listingid?.toString());
  ...
  const counterOfferStatusHandler = async (offerId: string, status: string) => {
    const input = { offerId, status }
    await useUpdatePropertyCounterOffer.mutateAsync(input)
    refetchCoutnerOffer()
    // queryClient.invalidateQueries({queryKey:[ 'propertyCounterOffers' , claimedProperty?.id?.toString(), claimedProperty?.listingid?.toString()]});
  }
          ...
          <>
            ...
            <section className="bg-white rounded-md px-4 py-6">
              ...
              <section className="flex flex-wrap gap-5 items-center">
                {propertyCounterOffers?.map((ofc, index) => {
                  const { offer } = ofc
                  return (
                    <BuyerOfferCard
                      onClick={() => handleOfferClick(offer)}
                      viewCounterOffer = {() => viewCounerOfferDetail(ofc)}
                      ...
                      coutnerOfferStatusHandler={counterOfferStatusHandler}
