...
const PropertyDetailsItem = () => {
  ...
  const handleEditPropertyClick = () => {
    router.push(`/dashboard/buy/${engagedProperty?.selectedProperty?.id}`)
  }
  ...
  return (
    <section className="flex-wrap flex items-center bg-[#0A0A0A] rounded-xl py-6 px-5 w-full gap-12">
      <section className="rounded-lg h-[12rem] overflow-hidden relative w-[30%]">
        {engagedProperty?.selectedProperty?.propertyImage ? (
          <Image
            src={engagedProperty.selectedProperty.propertyImage}
            alt={engagedProperty.selectedProperty.propertyImage}
            fill
            priority
            unoptimized
        ...
        <section className="flex items-center justify-between">
          <section className="w-3/5">
            <h2 className="font-light text-white text-md w-11/12">
              {engagedProperty?.engagedProperty?.engagement?.propertyName}
            </h2>
            <h3 className="text-[#BABABA] text-base">
              {engagedProperty.engagedProperty?.engagement?.propertyAddress} (
              {engagedProperty.engagedProperty?.engagement?.city}),{' '}
              {engagedProperty.engagedProperty?.engagement?.zipCode}
            </h3>
            <h1 className="text-white font-bold text-xl">
              {formatNumber(+engagedProperty.engagedProperty?.engagement?.price || 100)}
            </h1>
          </section>
          <section className="">
            <CircularProgressBar
