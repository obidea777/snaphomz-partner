  ...
  return (
    <section className={cn('w-[26rem] rounded-2xl overflow-hidden', className)}>
      <section className="h-[12rem] overflow-hidden relative w-full">
        ...
        <img
          src={property?.listing?.media?.primaryListingImageUrl || property?.mls_data?.data?.media?.primaryListingImageUrl}
      ...
      <section className="bg-[#0A0A0A] px-8 pt-4 pb-8 rounded-b-2xl ">
        <h2 className="text-[#F7F2EB] font-semibold text-[1.8rem]">
          {formatNumber(property?.listing?.leadTypes?.mlsListingPrice|| property?.mls_data?.data?.listPrice ||  0)}
        </h2>
        <div className="h-16 mb-2">
          {/* <p className="text-lg font-normal mt-2 text-[#F7F2EB]">
            {shortenAddress(property?.listing?.courtesyOf || property?.mls_data?.data?.courtesyOf)}
          </p> */}
          <p className="text-lg font-normal mt-2 text-[#F7F2EB]">
            {shortenAddress(property?.listing?.address?.unparsedAddress || property?.mls_data?.data?.address?.unparsedAddress)}
          </p>
        </div>
